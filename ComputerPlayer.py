...
class ComputerPlayer(Player):
    ...
    def addToGuessList(self,r,c,s):#makes sure a guess is legal before adding it to the guess list returns tru if the guess is a hit
        if -1 <  r < 10 and -1 <  c < 10 and self.shotGrid.returnLocation(r, c) == "~" and not self.isInGuessList(r, c):
            if s:
                self.guessList.insert(0,Shot("g", r, c))
            else:
                self.guessList.append(Shot("g", r, c))
        elif self.shotGrid.returnLocation(r, c) == "h" :
            return True
    ...
    def takeTurn(self, otherPlayer):
        for row in range(10):
            ...
            for col in range(10):
                ...
                if self.gridShots.returnLocation(row, col) != "h" or col == 9:
                    if hitCount >= 2:
                        self.addToGuessList(row, col+1, True)
                        self.addToGuessList(row, col-hitCount, True)
        ...
        for col in range(10):
            ...
            for row in range(10):
                ...
                if self.gridShots.returnLocation(row, col) != "h" or row == 9:
                    if hitCount>=2:
                        self.addToGuessList(row + 1,col,True)
                        self.addToGuessList(row - hitCount, col,True)
                        for x in range(hitCount):
        ...
        for row in range(10):
            for col in range(10):
                if self.gridShots.returnLocation(row, col) == "h":
                    self.addToGuessList(row + 1, col, False)
                    self.addToGuessList(row - 1, col, False)
                    self.addToGuessList(row , col + 1 , False)
                    self.addToGuessList(row , col - 1, False)
        if not bool(self.guessList) :
            self.randTurn(otherPlayer)
        else:
            self.Turn(otherPlayer,self.guessList[0].getRow(),self.guessList[0].getCol())
    ...
    def randTurn(self,otherPlayer):
        ...
        while (not self.gridShots.isSpaceWater(guess//10,guess%10)):#makes sure guess is not one previously guessed
            guess = random.randrange(0, 100)
        self.Turn(otherPlayer, guess//10, guess%10)
